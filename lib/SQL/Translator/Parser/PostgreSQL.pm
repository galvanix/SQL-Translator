...
{ my ( %tables, @views, @triggers, $table_order, $field_order, @table_comments, %enums) }
...
create : CREATE unique(?) /(index|key)/i index_name /on/i table_id using_method(?) '(' expression(s /,/) ')' where_predicate(?) ';'
    {
        my $table_info  = $item{'table_id'};
        my $schema_name = $table_info->{'schema_name'};
        my $table_name  = $table_info->{'table_name'};
